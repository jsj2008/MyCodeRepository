...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    _springXLabelWidth =  _chartCavanWidth / ([xLabels count]-emptyCount);
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            x = 2 * _chartMarginLeft + (index * _xLabelWidth)-_xLabelWidth/2;
            x = index * _xLabelWidth + _chartMarginLeft + _xLabelWidth / 2.0 -_springXLabelWidth/2;
            NSInteger y = _chartMarginBottom + _chartCavanHeight;
            //y = _chartCavanHeight;
            
            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(x, y, (NSInteger) _springXLabelWidth, (NSInteger) _chartMarginBottom)];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    BOOL handle = [self touchPoint:touches withEvent:event];
    handle?NSLog(@"touch line"):(handle = [self touchKeyPoint:touches withEvent:event]);
    handle?NSLog(@"touch point"):[super touchesBegan:touches withEvent:event];
}

- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    BOOL handle = [self touchPoint:touches withEvent:event];
    handle?NSLog(@"touch line"):(handle=[self touchKeyPoint:touches withEvent:event]);
    handle?NSLog(@"touch point"):[super touchesMoved:touches withEvent:event];
}
...
- (BOOL)touchKeyPoint:(NSSet *)touches withEvent:(UIEvent *)event {
    // Get the point user touched
    UITouch *touch = [touches anyObject];
    CGPoint touchPoint = [touch locationInView:self];
...
- (void)strokeChart {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        if (chartData.isDash) {
            CGFloat lineLength = 5,lineSpace = 2;
            if (chartData.lineLength != 0.f) {
                lineLength = chartData.lineLength;
            }
            if (chartData.lineSpace != 0.f) {
                lineSpace = chartData.lineSpace;
            }
            chartLine.lineDashPattern = [NSArray arrayWithObjects:[NSNumber numberWithFloat:lineLength], [NSNumber numberWithFloat:lineSpace], nil];
        }
    ...
    if (self.isShowCoordinateAxis) {
        ...
        if (self.showLabel) {
            ...
            for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
                ...
                CGContextAddLineToPoint(ctx, /*point.x + 2 +*/ xAxisWidth, point.y);
                CGContextStrokePath(ctx);
            }
        }
