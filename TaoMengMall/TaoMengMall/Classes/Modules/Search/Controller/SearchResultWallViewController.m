...
@interface SearchResultWallViewController ()
...
@implementation SearchResultWallViewController
    ...
    for (int i = 0; i < self.keys.count; i++) {
        ...
        [label bk_whenTapped:^{
            ...
            if (self.keys.count == 0) {
                [[TTNavigationService sharedService] openUrl:LOCALSCHEMA(@"search?key=")];
                //[self clickback];
            }else{
...
- (UIScrollView*)searchScrollView
{
    if (!_searchScrollView) {
        UIScrollView *scrollview = [[UIScrollView alloc]initWithFrame:CGRectMake(12, 7+STATUSBAR_HEIGHT, SCREEN_WIDTH-12-56, 30)];
        scrollview.backgroundColor = Color_Gray(238);
        ...
        [_searchScrollView bk_whenTapped:^{
            ...
            [[TTNavigationService sharedService] openUrl:LOCALSCHEMA(@"search?key=%@",self.key)];
            //[self clickback];
        }];
    }
...
- (UIView*)headerBgView
{
    if (!_headerBgView) {
        ...
        _headerBgView.backgroundColor = Color_White;
    }
...
{
    if (!_cancelButton) {
        _cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(SCREEN_WIDTH-50, STATUSBAR_HEIGHT+6, 44, 32)];
...
- (CGSize)sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return CGSizeMake(SCREEN_WIDTH, [WallItemCell heightForCell:wallItem]);
}
