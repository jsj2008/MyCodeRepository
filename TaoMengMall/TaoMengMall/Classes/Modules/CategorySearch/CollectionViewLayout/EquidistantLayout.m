...
@interface EquidistantLayout ()
@property (nonatomic, assign) CGFloat contentHeight;
@end

@implementation EquidistantLayout
...
-(CGSize)collectionViewContentSize
{
    ...
    for (int section = 0; section<sections; section++) {
        ...
        if (headAtt) height+= headAtt.frame.size.height;
        
        UICollectionViewLayoutAttributes * footAtt=[self layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionFooter atIndexPath:indexPath];
        if (footAtt) height+= footAtt.frame.size.height;

        height += [self getSectionHeight:section forMaxWidth:SCREEN_WIDTH];
    }
    size.height = height;
    return size;
    ...
    for (int section=0; section<sections; section++)
    {
        ...
        for (int row=0; row<rows; row++)
        {
            ...
            if ([self.delegate respondsToSelector:@selector(collectionView:layout:itemSpacingForSectionAtIndex:)]) {
                maximumSpacing = [self.delegate collectionView:self.collectionView layout:self itemSpacingForSectionAtIndex:section];
            }
