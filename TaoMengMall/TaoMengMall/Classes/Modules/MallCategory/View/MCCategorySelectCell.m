...
@interface MCCategorySelectCell()
...
@implementation MCCategorySelectCell
...
- (void)buttonClick:(UIButton *)button needDelegate:(BOOL)need
{
    ...
    if (button == self.selectButton && (![button.titleLabel.text isEqualToString:@"价格"])) {
        ...
        if (need) {
            ...
            if ([self.delegate respondsToSelector:@selector(cellChooseIndex:withPriceUpOrDown:keyName:keyValue:)]) {
                [self.delegate cellChooseIndex:button.tag withPriceUpOrDown:_temp keyName:key.name keyValue:option.value];
            }
        }
        // 更新价格高低图标
        if (_temp % 2 != 0) {
