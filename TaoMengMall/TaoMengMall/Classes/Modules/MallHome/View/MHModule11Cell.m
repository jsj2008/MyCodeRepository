...
@interface MHModule11Cell ()
@property (nonatomic, strong) NSMutableArray *moduleViews;
@end

@implementation MHModule11Cell
    ...
    if (!_moduleViews) {
        ...
        for (int i = 0; i<4; i++) {
            ...
            if (i == 0) {
                ...
                moduleView = [[MHSingleImageView alloc]initWithFrame:CGRectMake(PADDING, (height-PADDING)/2+2*PADDING, (height-PADDING)/2, (height-PADDING)/2)];
                moduleView2Height = (SCREEN_WIDTH - width)/item.ar;
            }else if (i == 2){
                moduleView = [[MHSingleImageView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH - width - 2*PADDING - (height-PADDING)/2, (height-PADDING)/2+2*PADDING, (height-PADDING)/2, (height-PADDING)/2)];
            }else if (i == 3){
