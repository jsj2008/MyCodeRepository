//
//  DSDistributorListViewController.m
//  CarKeeper
//
...
@interface DSDistributorListViewController ()<UISearchBarDelegate>
@property (nonatomic, strong) UISearchBar *searchBar;
@property (nonatomic, strong) NSMutableArray *list;
@property (nonatomic, strong) NSString *keyword;
@end

@implementation DSDistributorListViewController
...
- (void)loadData
{
    ...
    if (self.loadingType == kLoadMore) {
        ...
        [DistributorPromoteRequest getDistributorListWithParams:params success:^(DSDistributorListResultModel *resultModel) {
            if (resultModel) {
        ...
        [DistributorPromoteRequest getDistributorListWithParams:params success:^(DSDistributorListResultModel *resultModel) {
            ...
            if (resultModel.list.count == 0) {
                [self showEmptyTips:@"当前没有相关分销商哦" ownerView:self.tableView offsetTop:0];
            }
            [self finishRefresh];
            [self reloadData];
        } failure:^(StatusModel *status) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 0) {
        DSDistributorCell *cell = [DSDistributorCell dequeueReusableCellForTableView:tableView];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger section = indexPath.section;
    if (section == 0) {
        height = [DSDistributorCell heightForCell:[self.list safeObjectAtIndex:row]];
    }
    return height;
}
