//
//  DSDistributorPromoteViewController.m
//  CarKeeper
//
...
@interface DSDistributorPromoteViewController ()<UITextFieldDelegate>
...
@implementation DSDistributorPromoteViewController
...
- (void)loadData
{
    ...
    [DistributorPromoteRequest getDistributorPromoteDataWithParams:params success:^(DSDistributorPromoteResultModel *resultModel) {
...
- (UITextField *)phoneTextField {
    ...
    if ( !_phoneTextField ) {
        ...
        _phoneTextField.placeholder = @" 请输入分销商手机号";
...
- (UITextField *)nameTextField {
    ...
    if ( !_nameTextField ) {
        _nameTextField = [[UITextField alloc] initWithFrame:CGRectMake(52, self.iconImageView.bottom + 116, SCREEN_WIDTH - 52 - 52, 38)];
        _nameTextField.borderStyle = UITextBorderStyleRoundedRect;
        _nameTextField.placeholder = @" 请输入分销商姓名";
...
- (UIButton *)submitButton {
    ...
    if ( !_submitButton ) {
        ...
        button.frame = CGRectMake(52, self.iconImageView.bottom + 164, SCREEN_WIDTH - 104, 38);
...
- (void)handleCaptchaButton {
    ...
    NSDictionary *params = @{@"phone":phoneText,@"type":@1};
    
    weakify(self);
    [DistributorPromoteRequest getDistributorCaptchaWithParams:params success:^{
...
- (void)handleSubmitButton {
    ...
    DBG(@"next %@ %@ %@", self.phoneTextField.text, self.nameTextField.text, self.captchaTextField.text);
    ...
    [DistributorPromoteRequest inviteDistributorWithParams:params success:^{
        [self showNotice:@"邀请分销商成功"];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    DBG(@"next %@ %@ %@ %@", phoneText, nameText, captchaText, string);
    
    if ( !IsEmptyString(phoneText) && !IsEmptyString(nameText) && !IsEmptyString(captchaText) ) {
        [self setSubmitButtonEnabled:YES];
    } else {
        [self setSubmitButtonEnabled:NO];
    }
