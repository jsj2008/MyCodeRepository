...
+ (instancetype)photoWithImage:(UIImage *)image {
    return [[ITPhoto alloc] initWithImage:image];
}

+ (instancetype)photoWithURL:(NSURL *)url {
    return [[ITPhoto alloc] initWithURL:url];
}

+ (instancetype)photoWithAsset:(PHAsset *)asset targetSize:(CGSize)targetSize {
    return [[ITPhoto alloc] initWithAsset:asset targetSize:targetSize];
}

+ (instancetype)videoWithURL:(NSURL *)url {
    return [[ITPhoto alloc] initWithVideoURL:url];
}
                                                  ...
                                                  progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                                                      if (expectedSize > 0) {
                                                          ...
                                                          NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:
                                                                                ...
                                                                                self, @"photo", nil];
                                                          [[NSNotificationCenter defaultCenter] postNotificationName:ITPHOTO_PROGRESS_NOTIFICATION object:dict];
                                                      }
                                                  }
...
- (void)postCompleteNotification {
    [[NSNotificationCenter defaultCenter] postNotificationName:ITPHOTO_LOADING_DID_END_NOTIFICATION
                                                        object:self];
}
