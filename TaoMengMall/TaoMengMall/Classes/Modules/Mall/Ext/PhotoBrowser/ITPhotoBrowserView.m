...
@implementation ITPhotoBrowserView
...
- (void)_initialisation {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleMWPhotoLoadingDidEndNotification:)
                                                 name:ITPHOTO_LOADING_DID_END_NOTIFICATION
                                               object:nil];
...
- (void)layoutVisiblePages {
    ...
    for (ITZoomingScrollView *page in _visiblePages) {
...
- (ITCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    ITCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <ITPhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[ITCaptionView alloc] initWithPhoto:photo];
        }
    }
    captionView.alpha = self.hideCaption ? 0 : 1; // Initial alpha
    return captionView;
}
...
- (void)loadAdjacentPhotosIfNecessary:(id<ITPhoto>)photo {
    ITZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <ITPhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
...
- (void)handleMWPhotoLoadingDidEndNotification:(NSNotification *)notification {
    id <ITPhoto> photo = [notification object];
    ITZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        if ([photo underlyingImage]) {
            // Successful load
...
- (void)tilePages {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
            ITZoomingScrollView *page = [self dequeueRecycledPage];
            if (!page) {
                page = [[ITZoomingScrollView alloc] initWithPhotoBrowser:self];
            }
            ...
            // Add caption
            ITCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
            if (captionView) {
...
- (ITZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    ITZoomingScrollView *thePage = nil;
    for (ITZoomingScrollView *page in _visiblePages) {
        if (page.index == index) {
            thePage = page; break;
        }
...
- (ITZoomingScrollView *)dequeueRecycledPage {
    ITZoomingScrollView *page = [_recycledPages anyObject];
    if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    id <ITPhoto> currentPhoto = [self photoAtIndex:index];
...
- (void)didSingleTapPhotoAtIndex:(NSInteger)index{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:didSingleTapPhotoAtIndex:)]) {
        [self.delegate photoBrowser:self didSingleTapPhotoAtIndex:index];
    }
}

- (void)didDoubleTapPhotoAtIndex:(NSInteger)index
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:didDoubleTapPhotoAtIndex:)]) {
        [self.delegate photoBrowser:self didDoubleTapPhotoAtIndex:index];
    }
}
@end
