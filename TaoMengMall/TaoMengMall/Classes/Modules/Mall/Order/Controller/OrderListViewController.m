...
@interface OrderListViewController () <ODLOperateCellDelegate>
...
- (void)loadData
{
    ...
    if ( kLoadMore == self.loadingType ) {
        ...
        [OrderRequest getOrderListMoreDataWithParams:params success:^(OrderListResultModel *resultModel) {
        ...
        [params setSafeObject:self.status forKey:@"status"];
        
        [OrderRequest getOrderListDataWithParams:params success:^(OrderListResultModel *resultModel) {
...
- (void) calculateCellType {
    ...
    for (ODLFeedModel *order in self.orders) {
        ...
        for (ODLSubOrderInfoModel *subOrder in order.orderInfos) {
            ...
            [rowCellsType addObject:@{@"type":CELL_TYPE_SHOP, @"shopName":subOrder.shopName?subOrder.shopName:@"",@"shopLink":subOrder.shopLink?subOrder.shopLink:@"",@"logo":subOrder.shopLogo?subOrder.shopLogo:@""}];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( orderFeed ) {
        ...
        if ([CELL_TYPE_EMPTY isEqualToString:cellType]) {
            ...
            height = [ODLShopCell heightForCell:cellTypeInfo[@"shopName"]?cellTypeInfo[@"shopName"]:@""];
