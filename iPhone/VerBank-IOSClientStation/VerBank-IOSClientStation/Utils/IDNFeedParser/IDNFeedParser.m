...
//	parser.feedParseType = ParseTypeItemsOnly;
	...
	[parser parseData:data fromUrl:url];
    for (IDNFeedItem *item in parser.feedItems) {
        [item setMessageTitle:parser.feedInfo.title];
    }
			...
			switch (feedType) {
				...
				case FeedTypeRSS1: {
					...
					if (!processed) {
                        ...
                        else if ([currentPath isEqualToString:@"/rdf:RDF/item/pubDate"]) {
                            if (processedText.length > 0) parsingItem.date = [NSDate dateFromInternetDateTimeString:processedText formatHint:DateFormatHintRFC822]; processed = YES;
                        }
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
	...
	if(self)
	{
        ...
        _messageTitle = [aDecoder decodeObjectForKey:@"messageTitle"];
...
- (void)encodeWithCoder:(NSCoder *)aCoder
{
    ...
    [aCoder encodeObject:_messageTitle forKey:@"messageTitle"];
}
