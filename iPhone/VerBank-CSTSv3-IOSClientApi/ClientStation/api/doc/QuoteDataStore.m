...
static     NSMutableArray * quoteEventListenerArray;
        ...
        for (Instrument *instrument in list) {
            JEDI_SYS_Try{
                ...
                if (instrumentUtil != nil) {
                    CDS_PriceSnapShot *pss = [instrumentUtil getPriceSnapShotWithAccountID:[[ClientAPI getInstance] getAccount]];
                    if (pss != nil) {
    ...
    @synchronized(lock){
        ...
        if (pss == nil) {
            @try {
                ...
                if (instUtil != nil) {
                    pss = [instUtil getPriceSnapShotWithAccountID:[[ClientAPI getInstance] getAccount]];
                    //                    [self recQuoteData:pss];
                }
                return pss;
            }@catch (NSException *exception) {
                NSLog(@"*** get snapshot err !!! ***");
                return nil;
            }
        } else{
            return pss;
