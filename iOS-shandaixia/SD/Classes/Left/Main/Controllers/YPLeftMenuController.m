...
@property (nonatomic,weak) SDLeftHeaderView *leftHeaderView;
@property (nonatomic,weak) SDLeftDetailView *leftDetailView;
...
//    [self createUI];
    ...
    [SDNotificationCenter addObserver:self selector:@selector(loadLeftNavBar) name:SDLoginSuccessNotification object:nil];
    
    
    [SDNotificationCenter addObserver:self selector:@selector(loadLeftNavBar) name:SDReceivedPushingNotification object:nil];
...
- (void)addContent{
    ...
    //用户头像
    CGFloat headerH = 390 * SCALE;
    CGFloat headerW = SCREENWIDTH - 148 * SCALE;
    SDLeftHeaderView *leftHeaderView = [[SDLeftHeaderView alloc] init];
    
    leftHeaderView.frame = CGRectMake(0, 0, headerW, headerH);
    self.leftHeaderView = leftHeaderView;
    [self.view addSubview:leftHeaderView];
    
    //细节
    SDLeftDetailView *leftDetailView = [[SDLeftDetailView alloc] init];

    self.leftDetailView = leftDetailView;
    leftDetailView.frame = CGRectMake(0, headerH, headerW, SCREENHEIGHT - headerH);
    [self.view addSubview:leftDetailView];
...
- (void)loadLeftNavBar{
    ...
    [SDLeftNavBar leftNavBarFinishedBlock:^(id object) {
        
        self.leftHeaderView.leftNavBar = object;
        self.leftDetailView.leftNavBar = object;
