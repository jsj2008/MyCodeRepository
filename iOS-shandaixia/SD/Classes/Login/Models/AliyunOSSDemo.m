...
#define bucket @"zhixun-test"
#define endPoint @"http://oss-cn-shanghai.aliyuncs.com"
...
NSString * const AccessKey = @"LTAItzEI4cptl4s8";
NSString * const SecretKey = @"qQuWRr4gnyCt2mjYJXPAPhSy3mNpyx";
...
- (void)listObjectsInBucket {
    ...
    [getBucketTask continueWithBlock:^id(OSSTask *task) {
        if (!task.error) {
            ...
            FDLog(@"get bucket failed, error: %@", task.error);
        }
        return nil;
    }];
}
...
- (NSString *)uploadQuestionImageWithImageArray:(NSArray *)imageArray {
    ...
    for (int i =0; i < imageArray.count; i++) {
        ...
        NSString *name = [NSString stringWithFormat:@"questionImage/%@%d.png",[NSString getTimestamp], i];

        put.objectKey = name;
        
        if (i == imageArray.count - 1) {
            [nameString appendString:[NSString stringWithFormat:@"%@", name]];
        } else {
            [nameString appendString:[NSString stringWithFormat:@"%@,", name]];
        }
        
        //    NSString * docDir = [self getDocumentDirectory];
        //    put.uploadingFileURL = [NSURL fileURLWithPath:[docDir stringByAppendingPathComponent:name]];
...
//    put.uploadingFileURL = [NSURL fileURLWithPath:[docDir stringByAppendingPathComponent:name]];
    ...
    [putTask continueWithBlock:^id(OSSTask *task) {
        ...
        if (!task.error) {
            ...
            [manager postWithURLString:@"/attach/headPortrait" finishedBlock:^(id object) {
                
                FDLog(@"上传头像 - %@",object[@"msg"]);
                
                NSString *str = object[@"msg"];
                
                if ([str containsString:@"成功"]) {
                    
                    [FDReminderView showWithString:@"上传头像成功"];
                    
                    [SDNotificationCenter postNotificationName:SDHeadImageChangedNotification object:nil];
                }else{
                
                    [FDReminderView showWithString:@"上传头像失败"];
                }
