...
import jedi.ex02.CSTS3.comm.ipop.C3_IP_BankServ1001;
import jedi.ex02.CSTS3.comm.ipop.C3_OPFather;
import jedi.ex02.CSTS3.comm.ipop.C3_OP_BankServ1001;
...
public class ClientTradeBankServ1001 extends ClientTradeFather {

	@Override
	protected C3_OPFather doTrade(C3_IPFather _ip, String aeid, String ipaddress) {

		C3_IP_BankServ1001 ip = (C3_IP_BankServ1001) _ip;
		C3_OP_BankServ1001 op = new C3_OP_BankServ1001(ip);
		IPFather fip = ip.formatIP();
		OPFather fop = StaticContext.getClientTradeCaptain().doSysTrade(fip,
				aeid, ipaddress);
		try {
			if (fop.isSucceed()) {
				op.parseFromSysData((OP_BankServ1001) fop);
			} else {
