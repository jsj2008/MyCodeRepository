...
import jedi.ex02.CSTS3.comm.ipop.C3_IP_TRADESERV5108;
import jedi.ex02.CSTS3.comm.ipop.C3_OPFather;
import jedi.ex02.CSTS3.comm.ipop.C3_OP_TRADESERV5108;
...
import jedi.v7.trade.comm.IPOP.OP_TRADESERV5108;

public class ClientTradeTRADESERV5108 extends ClientTradeFather {

	@Override
	protected C3_OPFather doTrade(C3_IPFather _ip, String aeid,
			String ipaddress) {
		C3_IP_TRADESERV5108 ip = (C3_IP_TRADESERV5108) _ip;
		C3_OP_TRADESERV5108 op = new C3_OP_TRADESERV5108(ip);
		IPFather fip = ip.formatIP();
		OPFather fop = StaticContext.getClientTradeCaptain().doSysTrade(fip,
				aeid, ipaddress);
		try {
			if (fop.isSucceed()) {
				op.parseFromSysData((OP_TRADESERV5108) fop);
			} else {
