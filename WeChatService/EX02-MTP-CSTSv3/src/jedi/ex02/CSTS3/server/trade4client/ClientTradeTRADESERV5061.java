...
import jedi.ex02.CSTS3.comm.ipop.C3_IP_TRADESERV5061;
import jedi.ex02.CSTS3.comm.ipop.C3_OPFather;
import jedi.ex02.CSTS3.comm.ipop.C3_OP_TRADESERV5061;
...
public class ClientTradeTRADESERV5061 extends ClientTradeFather {

	@Override
	protected C3_OPFather doTrade(C3_IPFather _ip, String aeid,
			String ipaddress) {
		C3_IP_TRADESERV5061 ip = (C3_IP_TRADESERV5061) _ip;
		C3_OP_TRADESERV5061 op = new C3_OP_TRADESERV5061(ip);
		IPFather fip = ip.formatIP();
		OPFather fop = StaticContext.getClientTradeCaptain().doSysTrade(fip,
				aeid, ipaddress);
		try {
			if (fop.isSucceed()) {
				op.parseFromSysData((OP_TRADESERV5061) fop);
			} else {
