...
import jedi.ex02.CSTS3.comm.ipop.C3_IP_TRADESERV5102;
import jedi.ex02.CSTS3.comm.ipop.C3_OPFather;
import jedi.ex02.CSTS3.comm.ipop.C3_OP_TRADESERV5102;
...
public class ClientTradeTRADESERV5102 extends ClientTradeFather {

	@Override
	protected C3_OPFather doTrade(C3_IPFather _ip, String aeid,
			String ipaddress) {
		C3_IP_TRADESERV5102 ip = (C3_IP_TRADESERV5102) _ip;
		C3_OP_TRADESERV5102 op = new C3_OP_TRADESERV5102(ip);
		IPFather fip = ip.formatIP();
		IP_TRADESERV5102 tip=(IP_TRADESERV5102) fip;
		tip.setAeid(aeid);
		OPFather fop = StaticContext.getClientTradeCaptain().doSysTrade(fip,
				aeid, ipaddress);
		try {
			if (fop.isSucceed()) {
				op.parseFromSysData((OP_TRADESERV5102) fop);
			} else {
