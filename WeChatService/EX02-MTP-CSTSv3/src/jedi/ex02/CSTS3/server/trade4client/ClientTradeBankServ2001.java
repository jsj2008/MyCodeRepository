...
import jedi.ex02.CSTS3.comm.ipop.C3_IP_BankServ2001;
import jedi.ex02.CSTS3.comm.ipop.C3_OPFather;
import jedi.ex02.CSTS3.comm.ipop.C3_OP_BankServ2001;
import jedi.ex02.CSTS3.server.StaticContext;
import jedi.v7.bankserver.comm.BankServIPFather;
...
public class ClientTradeBankServ2001 extends ClientTradeFather {

	@Override
	protected C3_OPFather doTrade(C3_IPFather _ip, String aeid, String ipaddress) {
		C3_IP_BankServ2001 ip = (C3_IP_BankServ2001) _ip;
		C3_OP_BankServ2001 op = new C3_OP_BankServ2001(ip);
		IPFather fip = ip.formatIP();
		OPFather fop = StaticContext.getClientTradeCaptain().doSysTrade(fip,
				aeid, ipaddress);
		try {
			if (fop.isSucceed()) {
				op.parseFromSysData((OP_BankServ2001) fop);
			} else {
