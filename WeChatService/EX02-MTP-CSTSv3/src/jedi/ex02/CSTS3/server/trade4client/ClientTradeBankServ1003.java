...
import jedi.ex02.CSTS3.comm.ipop.C3_IP_BankServ1003;
import jedi.ex02.CSTS3.comm.ipop.C3_OPFather;
import jedi.ex02.CSTS3.comm.ipop.C3_OP_BankServ1003;
...
public class ClientTradeBankServ1003 extends ClientTradeFather {

	@Override
	protected C3_OPFather doTrade(C3_IPFather _ip, String aeid, String ipaddress) {

		C3_IP_BankServ1003 ip = (C3_IP_BankServ1003) _ip;
		C3_OP_BankServ1003 op = new C3_OP_BankServ1003(ip);
		IPFather fip = ip.formatIP();
		OPFather fop = StaticContext.getClientTradeCaptain().doSysTrade(fip,
				aeid, ipaddress);
		try {
			if (fop.isSucceed()) {
				op.parseFromSysData((OP_BankServ1003) fop);
			} else {
