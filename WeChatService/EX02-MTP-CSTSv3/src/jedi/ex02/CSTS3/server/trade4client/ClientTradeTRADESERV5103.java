...
import jedi.ex02.CSTS3.comm.ipop.C3_IP_TRADESERV5103;
import jedi.ex02.CSTS3.comm.ipop.C3_OPFather;
import jedi.ex02.CSTS3.comm.ipop.C3_OP_TRADESERV5103;
...
public class ClientTradeTRADESERV5103 extends ClientTradeFather {

	@Override
	protected C3_OPFather doTrade(C3_IPFather _ip, String aeid,
			String ipaddress) {
		C3_IP_TRADESERV5103 ip = (C3_IP_TRADESERV5103) _ip;
		C3_OP_TRADESERV5103 op = new C3_OP_TRADESERV5103(ip);
		IPFather fip = ip.formatIP();
		IP_TRADESERV5103 tip=(IP_TRADESERV5103) fip;
		tip.setAeid(aeid);
		OPFather fop = StaticContext.getClientTradeCaptain().doSysTrade(fip,
				aeid, ipaddress);
		try {
			if (fop.isSucceed()) {
				op.parseFromSysData((OP_TRADESERV5103) fop);
			} else {
