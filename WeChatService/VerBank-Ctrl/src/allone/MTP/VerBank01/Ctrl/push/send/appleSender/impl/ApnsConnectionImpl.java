...
import static allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ApnsConstants.CHARSET_ENCODING;
import static allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ApnsConstants.ERROR_RESPONSE_BYTES_LENGTH;
import static allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ApnsConstants.PAY_LOAD_MAX_LENGTH;
...
import allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.Command;
import allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ErrorResponse;
...
public class ApnsConnectionImpl implements IApnsConnection {
	...
	public void sendNotification(PushNotification notification) {
		...
		synchronized (lock) {
			...
			if (!isSuccessful) {
				LogProxy.ErrPrintln(String.format("%s Notification send failed. %s", connName, notification));
				return;
			} else {
				LogProxy.OutPrintln(String.format("%s Send success. count: %s, notificaion: %s", connName, notificaionSentCount.incrementAndGet(), notification));
	...
	private Socket createNewSocket() throws IOException, UnknownHostException {
		if (logger.isDebugEnabled()) {
			LogProxy.ErrPrintln(connName + " create a new socket.");
				...
				try {
					...
					if (size == res.length && command == Command.ERROR) {
						...
						if (logger.isInfoEnabled()) {
							LogProxy.OutPrintln(String.format("%s Received error response. status: %s, id: %s, error-desc: %s", connName, status, errorId, ErrorResponse.desc(status)));
						}
						...
						LogProxy.OutPrintln(connName + " Unexpected command or size. commend: " + command + " , size: " + size);
					}
				} catch (Exception e) {
					// logger.error(connName + " " + e.getMessage(), e);
					LogProxy.ErrPrintln(e.getMessage());
					LogProxy.OutPrintln(connName + " " + e.getMessage());
				} finally {
					/**
