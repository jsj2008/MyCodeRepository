...
import static allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ApnsConstants.HOST_DEVELOPMENT_ENV;
import static allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ApnsConstants.HOST_PRODUCTION_ENV;
import static allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ApnsConstants.PORT_DEVELOPMENT_ENV;
import static allone.MTP.VerBank01.Ctrl.push.send.appleSender.model.ApnsConstants.PORT_PRODUCTION_ENV;
...
public class ApnsConnectionPool implements Closeable {
	...
	private ApnsConnectionPool(ApnsConfig config, SocketFactory factory) {
		...
		for (int i = 0; i < poolSize; i++) {
			...
			IApnsConnection conn = new ApnsConnectionImpl(this.factory, host, port, config.getRetries(), config.getCacheLength(), config.getName(), connName,
					config.getIntervalTime(), config.getTimeout());
			connQueue.add(conn);
		}
	}
