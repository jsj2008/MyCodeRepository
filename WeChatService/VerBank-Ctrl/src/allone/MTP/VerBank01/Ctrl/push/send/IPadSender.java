...
public class IPadSender implements I_Sender {
	...
	public void sendInfoToUser(String _accountID, String _info) {
		...
		String token = RegisterDoc.getInstance().getIPadDevice(_accountID);
		...
		LogProxy.OutPrintln("IPad Sender accountID : " + _accountID);
		LogProxy.OutPrintln("IPad Sender sendToToken : " + token);
		LogProxy.OutPrintln("IPad Sender message : " + _info);
	...
	public void sendInfoToAllUser(String info) {
		Collection<String> iOSAccounts = RegisterDoc.getInstance().getAllIPadAccount();
	...
	private IApnsService getApnsService(String accountID) {
		...
		if (apnsService == null) {
			...
			InputStream is = StaticContext.getSendConfig().getiPadKeyStream();
			config.setKeyStore(is);
			config.setDevEnv(StaticContext.getSendConfig().isiPadIsDevelop());
			config.setPassword(StaticContext.getSendConfig().getiPadKeyPwd());
	...
	private void checkPort(String ip, int port) {
		try {
			...
			LogProxy.OutPrintln("ipad : " + ip + " : " + port + " is open");
			s.close();
		} catch (Exception e) {
			LogProxy.OutPrintln("ipad : " + ip + " : " + port + " is not open");
		}
	}
}
